...
\skillListStart
  \item \textbf{Programming:} Python (\textbf{Pandas} for SQL-style joins, group-by aggregation, filtering to process CSV files), C/C++, MATLAB, Julia, Arm Assembly, Perl
  \item \textbf{Data Processing \& Automation:} Built  CI/CD (Jenkins) pipelines to organize large IC/IP regression suites, \textbf{enabling same-day dashboards vs.\ 1–2-day manual consolidation}; Makefile-driven build/test flows;
  \item \textbf{ML / Optimization:} PyTorch, GRU-RNN, MLP, Attention Mechanism, Convex Optimization (fractional \& quadratic-transform methods), Sampling-based Source Coding
  \item \textbf{Parallel Programming:} Experience with \textbf{multithreading} (OpenMP-style parallel loops, SIMD intrinsics, custom thread management) and \textbf{multi-queue scheduling} strategies for scalable workload distribution.
